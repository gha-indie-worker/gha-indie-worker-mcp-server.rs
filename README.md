...
The exact `gha-indie-worker/gha-indie-worker-mcp-server.rs` organization, repository, service,
...
- `github_posture` — the exact gha-indie-worker repository and CI posture;
...
- `supabase_posture` — the configured gha-indie-worker Supabase health boundary;
- `neon_posture` — exact Neon organization/project and branch metadata;
- `cloudflare_posture` — the configured gha-indie-worker zone and DNS posture;
- `k8s_posture` — the exact gha-indie-worker namespace in the
  `ORESoftware/k8s-cluster` deployment plane;
- `nats_posture` — the gha-indie-worker service and dependency subjects; and
...
local stdio process accepts no bearer token. The `gha-indie-worker-mcp-http` binary
...
public resource as audience, requires the gha-indie-worker project realm, AAL2,
`mcp:read` plus `gha-indie-worker:inspect`, and a `gha-indie-worker_viewer` or
`gha-indie-worker_operator` role. It never forwards a caller token upstream. Exact
...
- Kubernetes: `ORE_MCP_K8S_ENABLED=1`, optionally
  `ORE_MCP_K8S_NAMESPACE` (otherwise the exact `gha-indie-worker` namespace); and
- NATS: `ORE_MCP_NATS_URL`, scoped to
  `gha-indie-worker.mcp.service.read.v1` and `gha-indie-worker.mcp.dependencies.read.v1`.
