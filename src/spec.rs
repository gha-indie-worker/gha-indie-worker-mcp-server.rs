//! Immutable gha-indie-worker identity and provider-operation contract.
...
pub fn org_spec() -> OrgSpec {
    ...
    OrgSpec {
        organization: "gha-indie-worker",
        repository: "gha-indie-worker/gha-indie-worker-mcp-server.rs",
        service_name: "gha-indie-worker-mcp-server",
        package_name: "gha-indie-worker-mcp-server",
...
mod tests {
    ...
    fn scope_and_provider_contract_are_exact() {
        let spec = org_spec();
        assert_eq!(spec.organization, "gha-indie-worker");
        assert_eq!(
            spec.repository,
